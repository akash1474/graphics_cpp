...
std::vector<double> Matrix::diagonal(){
	if(this->rows==this->cols){
		printf("ERROR_DIAGONAL: Matrix is not a square matrix (nxn)");
		throw "ERROR_DIAGONAL: Matrix is not a square matrix (nxn)";
	}
...
Matrix Matrix::dot(const Matrix& m1,const Matrix& m2){
	if(m1.cols!=m2.rows){
		...
		throw "Matrix not compatible for dot product";
	}
